...
class Block(nn.Module):
    def __init__(self, config, layer_idx):
        ...
        self.has_mlp = layer_idx < config.n_layer - 3  # skip MLP in last 3 layers (optimal from run 333)
        if self.has_mlp:
...
SCALAR_LR = 0.2         # optimal (revert from 0.15 test)
WEIGHT_DECAY = 0.1      # cautious weight decay for Muon
ADAM_BETAS = (0.9, 0.95) # Adam betas
WARMUP_RATIO = 0.005    # optimal near-zero warmup: maximize steps at peak LR
WARMDOWN_RATIO = 0.3    # optimal from Run 40
FINAL_LR_FRAC = 0.15    # optimal confirmed in Phase 5
...
with autocast_ctx:
    ...
    prepare.EVAL_TOKENS = 25000  # Extreme-minimal evaluation: ~5 steps, ~6-8s eval time, to avoid OOM on 4GB GPU
