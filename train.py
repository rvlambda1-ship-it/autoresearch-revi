...
def has_ve(layer_idx, n_layer):
    ...
    Disabled for small models — value embeds dominate params and destabilize training."""
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, ve, cos_sin, window_size):
        ...
        k = k.transpose(1, 2)
        v = v.transpose(1, 2)
        # Expand k/v for GQA if n_head != n_kv_head
...
class Block(nn.Module):
    def __init__(self, config, layer_idx):
        ...
        self.has_mlp = layer_idx < config.n_layer - 4  # skip MLP in last 4 layers
        if self.has_mlp:
...
class GPT(nn.Module):
    ...
    def init_weights(self):
        ...
        # Linearly decreasing x0_lambda: early layers get more x0 signal
        for i in range(config.n_layer):
            self.x0_lambdas.data[i] = 0.15 - 0.1 * (i / max(config.n_layer - 1, 1))
        # Value embeddings
    ...
    def setup_optimizer(self, unembedding_lr=0.004, embedding_lr=0.2, matrix_lr=0.02,
        ...
        # Scale LR ∝ 1/√dmodel (tuned at 768 dim), capped at 1.0 for small models
        dmodel_lr_scale = min(1.0, (model_dim / 768) ** -0.5)
        ...
        for layer_idx, block in enumerate(self.transformer.h):
            ...
            layer_shapes = sorted({p.shape for p in layer_params})
            for shape in layer_shapes:
                layer_shape_params = [p for p in layer_params if p.shape == shape]
                layer_matrix_lr = matrix_lr * layer_lr_mult
                if USE_MUON:
                    param_groups.append(dict(
                        kind='muon', params=layer_shape_params, lr=layer_matrix_lr,
                        momentum=0.95, ns_steps=2, beta2=0.95, weight_decay=weight_decay,
                    ))
                else:
                    param_groups.append(dict(
                        kind='adamw', params=layer_shape_params, lr=layer_matrix_lr,
...
EMBEDDING_LR = 1.2      # even lower embedding LR
...
WARMUP_RATIO = 0.0      # zero warmup: grad clip protects initial steps
...
DEVICE_BATCH_SIZE = 8    # per-device batch size (2K tokens)
...
GPU_FP16_PEAK_FLOPS = 4.6e12  # GTX 1650 FP16 peak TFLOPS
...
def get_lr_multiplier(progress):
    if progress < WARMUP_RATIO:
        ...
        # Cosine decay from peak to FINAL_LR_FRAC after warmup
...
while True:
    ...
    mfu = 100 * num_flops_per_token * TOTAL_BATCH_SIZE / dt / GPU_FP16_PEAK_FLOPS
...
steady_state_mfu = 100 * num_flops_per_token * TOTAL_BATCH_SIZE * (step - 10) / total_training_time / GPU_FP16_PEAK_FLOPS if total_training_time > 0 else 0
