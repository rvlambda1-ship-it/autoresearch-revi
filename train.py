...
class MLP(nn.Module):
    def __init__(self, config):
        super().__init__()
        # SwiGLU: gate and up projections, then down projection
        # Hidden dim ~8/3 * n_embd rounded to multiple of 64 for efficiency
        hidden = ((4 * config.n_embd * 2 // 3) + 63) // 64 * 64
...
EMBEDDING_LR = 1.0      # higher LR for unnormed embeddings
...
WARMUP_RATIO = 0.05     # shorter warmup: maximize steps at peak LR
WARMDOWN_RATIO = 0.3    # shorter warmdown: more time at full LR
FINAL_LR_FRAC = 0.15    # slightly higher min LR
